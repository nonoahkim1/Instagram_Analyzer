...
async function analyzeFolder() {
    ...
    const fileProcessors = {
        ...
        'post_comments_1.json': async (file) => {
            const post_comments_JSON = JSON.parse(await file.text());
            userData.post_comments = post_comments_JSON.map(fg => {
                const commentData = fg.string_map_data["Comment"];
                const comment = commentData ? decodeString(commentData.value) : '';
                const uri = fg.media_list_data[0].uri;
                const mediaOwner = fg.string_map_data["Media Owner"];
                const timestampData = fg.string_map_data["Time"];

                return {
                    username: mediaOwner ? mediaOwner.value : 'Unknown',
                    comment: comment || (uri ? `<img src="${uri}" alt="GIF">` : 'Avatar Sticker'),
                    timestamp: timestampData ? adjustTimestamp(timestampData.timestamp) : 0
                };
            });
        }
    };
    ...
    console.log(userData.post_comments); // Debug log
    ...
    if (Object.values(userData).some(arr => arr.length > 0)) {
        ...
        const linkMappings = {
            ...
            post_comments: 'postCommentsLink'
        };
